...
const Editor = ({ roomId, isOwner }: EditorProps) => {
  ...
  return (
    <div className={`h-full overflow-auto ${theme === 'dark' ? 'bg-dark text-light' : 'bg-white text-dark'}`}>
      <div className="mx-auto">
        ...
        />
        <div className={`shadow-lg overflow-hidden mt-2 ${theme === 'dark' ? 'bg-dark border-dark' : 'bg-white border-gray-200'}`}>
...
const EditorToolbar = ({ isOwner, onShare, onManageAccess, theme, toggleTheme, doc }: {isOwner:boolean, onShare:()=>void, onManageAccess:()=>void, theme:string, toggleTheme:()=>void, doc:Y.Doc}) => {
  const others = useOthers()

  return (
    <div className={`p-4 border-b ${theme === 'dark' ? 'bg-dark border-dark' : 'bg-white border-gray-200'} flex items-center justify-between`}>
      <div className='flex gap-4'>
        <AIFeaturesDropdown doc={doc} />
        {isOwner && (
          <Button variant="outline" size="sm" className={`button-dark`} onClick={onManageAccess}>
      ...
      <div className='flex items-center gap-4'>
        <AnimatedTooltip items={others.map(other => ({
          id: Number(other.connectionId),
          name: other.info.name || 'Anonymous',
          image: other.info.avatar || 'https://via.placeholder.com/150'
        }))} />
        <Button variant="outline" size="sm" className={`button-dark`} onClick={onShare}>
