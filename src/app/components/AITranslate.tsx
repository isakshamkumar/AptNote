...
const AITranslate: React.FC<{ isOpen: boolean; onClose: () => void, doc: Y.Doc }> = ({ isOpen, onClose, doc }) => {
  ...
  const handleTranslate = async (e: React.FormEvent) => {
    ...
    const userMessage = { id: Date.now(), content: `Translate to ${targetLanguage}: ${input}`, sender: 'user' as const };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`sm:max-w-[700px] h-[80vh] flex flex-col ${theme === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-white'}`}>
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">Translate Document with AI</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleTranslate} className="flex flex-col space-y-2">
          ...
          <div className="flex space-x-2">
            ...
            <Button type="submit" disabled={isPending || !input.trim() || !targetLanguage}>
              {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
            </Button>
          </div>
        </form>
        {messages.some(m => m.sender === 'ai') && (
          <Button onClick={handleCopy} className={`mt-2 ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : ''}`} variant="outline">
